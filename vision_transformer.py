...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape #（batch，序列长度，特征维度）
        # (B,N,C)->(B, N, 3, self.num_heads, self.head_dim) -> (3, B, self.num_heads, N, self.head_dim)
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0) #都变成  (B, self.num_heads, N, self.head_dim)
        q, k = self.q_norm(q), self.k_norm(k)

        if self.fused_attn: # 融合的注意力计算方法    softmax（QK/根号dk）*V
            x = F.scaled_dot_product_attention(
                q, k, v,
                dropout_p=self.attn_drop.p if self.training else 0.,
            )
        else:        # 传统的注意力方法    q先缩放 再和K点积
            q = q * self.scale
            attn = q @ k.transpose(-2, -1)   # k转置 交换最后和倒数第二个维度
            attn = attn.softmax(dim=-1)
            attn = self.attn_drop(attn)
            x = attn @ v
        # x 的形状还是保持(B, self.num_heads, N, self.head_dim)
        x = x.transpose(1, 2).reshape(B, N, C)
        # x 变回(B, N, self.num_heads, self.head_dim)再reshape
        x = self.proj(x)
        x = self.proj_drop(x)
        return x


class LayerScale(nn.Module):   # 对层之间的输出进行缩放
...
class Block(nn.Module):
    ...
    def __init__(
            ...
            dim,   
            num_heads,
            mlp_ratio=4.,    # MLP 层中隐藏层维度与输入维度的比例
            qkv_bias=False,
            qk_norm=False,
            proj_drop=0.,  # 表示在注意力机制和 MLP 层之间的投影操作中的 dropout 概率
            attn_drop=0.,  # 表示在注意力机制中的 dropout 概率
            init_values=None,
            drop_path=0.,  # 表示在残差连接路径中的 dropout 概率
            act_layer=nn.GELU,
            norm_layer=nn.LayerNorm,
            mlp_layer=Mlp,
    ):
